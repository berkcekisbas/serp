...
class EditConfigFiles extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tor:edit:configs';
    ...
    public function handle()
    {
        ...
        for ($x = 0; $x <= env('PROXY_INSTANCE_COUNT'); $x++) {
            ...
            $ex = $ssh->exec($command);
            $this->line($x." Numaralı servis dosyası düzenlendi");
            $ex = $ssh->exec('systemctl start tor@'.sprintf("%02d",$x));
            $this->line($x." Numaralı servis başlatıldı");
